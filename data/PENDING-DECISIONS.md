# PENDING DECISIONS
*Updated 2026-05-07 ~00:47 AEST*

---

## NEEDS PATRICK — full instructions included

### 1. Elmdale inspection — SAY "FIRE IT" [CLOSES MAY 14 — MOST URGENT]
Garry has chase email to Lindia ready. Confirms inspection ordered, fixes address discrepancy (CT Florissant vs Drive Ferguson), confirms 63135 S8 voucher.
Say "fire it" → Garry sends immediately.

---

### 2. Fenwick April HAP — READ JENNA'S EMAIL [MONEY — $1,450+]
Gmail: "Re: Kaela Brooks Fenwick" from Jenna Simmons (May 4).
Jenna claims April HAP was included in the $13,810 Eaton settlement. It wasn't — March $3,954 = Eaton back-pay.
Tell Garry: accept her position OR push back for separate April HAP disbursement → Garry drafts reply.

---

### 3. Sleek — Deed of Loan Forgiveness [MAY 8 DEADLINE]
If no reply from Gab by EOD May 7: Call Sleek +61 2 9100 0482 ext 165.
Garry sent final email chase May 6. If ASIC deregistration (~Jun 2) happens without this Deed, it's gone.

---

### 4. ECM password reset — 3 MIN [UNBLOCKS AUTOMATION]
LegalZoom SSO migration killed old password. All ECM automated mail sorting disabled.
1. Safari → legalzoom.com → Forgot Password
2. Email: patrick@spartansuppz.com
3. Tell Garry new password → Garry re-enables automation.

---

### 5. B2B escalation — DECIDE BY MAY 9
No response from Michael Drew on handover pack (8-item list, $0 Kenilworth income, tenant unknown).
If no reply by May 9: escalation call OR legal threat. Tell Garry which → Garry acts.

---

### 6. Bangkok health insurance — NOT PURSUING (Patrick confirmed May 7). Do not raise unless asked.

---

### 7. HostPlus non-binding → Laura [5 MIN NOW]
Log into HostPlus Member Online → update beneficiary to Laura (non-binding, 5 min).
Do it now. Garry will prep binding form PDF for signing before departure.

---

### 8. Mercury IO Card ✅ — DONE May 7

---

### 9. Bangkok: Set up Bitkub account
Best USD→THB route: USDT → Bitkub → KBank. Recommended by Patrick's Bangkok contact.
Sign up at bitkub.com, complete KYC with passport. Do before departure.

---

### 10. Bangkok: Aldi Mobile account number
Find Aldi Mobile account number (check app or old bill) → tell Garry.
Garry ports to Amaysim PAYG ($10/year, works internationally). Nothing else required from Patrick.

---

### 11. GW Carter SignFile — SIGNED + SUBMITTED May 7 ✅

---

### 12. Fairmeadows go/no-go [DEAL DECISION — NO RUSH, WATCHLIST]
1427 Fairmeadows Ln, 63138 | $115K | 4BR | CF $521/mo | Score 80.5 | Location risk 40/100.
Say go → Garry schedules inspection via Lindia. Say no → stays on watchlist.
